 ...
 *
 * @return string Empty string on failure, otherwise embedded media.
 */
function et_get_media_embedded_in_content( $post_id ) {
	$embeds = get_media_embedded_in_content(
		/**
		 * Catch the oembeds in the content using apply_filters( 'the_content', $content )
		 * because get_the_content() returns the unfiltered content
		 */
		apply_filters( 'the_content', get_the_content( $post_id ) ),
		array( 'iframe', 'video' ) // Catch only 'iframe' for youtube and vimeo and 'video' for self hosted videos.
	);
	...
	$embeds = array_map( 'trim', $embeds );

	return implode( '', $embeds );
}
